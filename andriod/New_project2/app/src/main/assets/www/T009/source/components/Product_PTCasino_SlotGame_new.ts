...
class Product_PTCasino_SlotGame{
	...
	toLoadProductList(data){
		...
		Api.ptcasino.product_list_new(data)
			.done((result)=>{
				...
				this.content.Product_List = [];
				result.data.map((element,idx) =>{
					element.game.forEach(el => {
						el.Category = result.data[idx].Category
					})
					return element.game;
				}).forEach(element => this.content.Product_List.push(...element));
	...
	toLoadProductListCallback(result){
		...
		var data = this.content.Product_List,
	...
	filterData(){
		...
		data.forEach((item, idx)=>{
			if(gameName != ""){
				if(item.Name.toLowerCase().indexOf(gameName.toLowerCase()) > -1) filter_data.push(item);
			}
			else if(gameId != ""){
				if(gameId.indexOf(item.Category) > -1) filter_data.push(item);
			}
			else {
				...
				.done((baseInfo)=>{
					if(Global.App.accessCheck(Com_Gametree_Cashap.SiteConfig.memberRuleId.PTCasino, baseInfo.memberLevel)){
						if(baseInfo.memberLevel == Global.MemberLevel.trial){
							...
							var url = this.loginUrl+"?gameCode="+item.GameId;
