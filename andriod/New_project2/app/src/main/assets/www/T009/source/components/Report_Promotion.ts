...
class Report_Promotion {
    formName: string;
    formElement: JQuery;

    messageNS: string = "Report_Promotion_History";
    ...
    afterInit() {
        ...
        $(this.contentTable).delegate("[data-cashap-id=depositID]", "click", (e: Event) => {
            e.preventDefault();
    ...
    submit() {
        ...
        if (new Date(form_data.startDate).valueOf() > new Date(form_data.endDate).valueOf()) {
            Global.Tips.systemTip((<ILanguage>Com_Gametree_Cashap.Language).getMessage_Translate(this.messageNS, "dateRangeInvalid1"));
            return;
        }
