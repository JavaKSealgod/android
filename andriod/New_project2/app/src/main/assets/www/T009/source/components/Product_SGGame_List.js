define('T009/source/components/Product_SGGame_List', ["require", "exports", "T009/source/modules/global", "Global/source/modules/api", "handlebars", "Global/libs/zepto.picLazyLoad"], function (require, exports, Global, Api, Handlebars) {
    "use strict";
    // import BackToTop = require("../../../Global/source/widgets/BackToTop");
    var Product_SGGame_List = /** @class */ (function () {
        function Product_SGGame_List(compOption) {
            this.messageNS = "";
            //标签列表总容器
        ...
        Product_SGGame_List.prototype.toLoadSiteMessage = function () {
            var _this = this;
        ...
        Product_SGGame_List.prototype.init = function () {
            var _this = this;
            //显示系统loading
            Global.Tips.showSystemLoading();
            Api.sggame.product_list()
                .done(function (data) {
                //隐藏系统loading
                Global.Tips.hideSystemLoading();
        ...
        Product_SGGame_List.prototype.productListLoadCallback = function (data) {
            var productList = data.data, allCount = 0;
            for (var i = 0, l = productList.length; i < l; i++) {
                productList[i].isCurrent = false;
                var item = productList[i].game;
                allCount += item.length;
                for (var x = 0, xl = item.length; x < xl; x++) {
                    item[x].imgURL = Com_Gametree_Cashap.Language.fileDomain + "/sggame/" + item[x].id + ".jpg";
                }
            }
            productList.unshift({
                catalogName: "全部",
        ...
        Product_SGGame_List.prototype.changeCategory = function (catalogId) {
            Global.Log.log("changeCategory ", catalogId);
            this.gameListData.forEach(function (item, idx) {
                item.catalogId === catalogId ? item.isCurrent = true : item.isCurrent = false;
            });
            //清空游戏名称搜索框内容
         ...
         * 游戏搜索
         */
        Product_SGGame_List.prototype.searchByName = function () {
        ...
        Product_SGGame_List.prototype.toOpenProductURL = function (gameId) {
            var _this = this;
        ...
        Product_SGGame_List.prototype.setGameList = function (productList) {
         ...
         * 创建html结构
         */
        Product_SGGame_List.prototype.setHtml = function (target, data, tpl) {
            var template = Handlebars.compile(tpl), html = template(data);
            target.html(html);
        };
        return Product_SGGame_List;
    }());
    return Product_SGGame_List;
});
//# sourceMappingURL=/SGMobile_H5App_V1/T009/source/components/Product_SGGame_List.js.map