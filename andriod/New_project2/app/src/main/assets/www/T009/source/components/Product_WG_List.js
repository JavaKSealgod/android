define('T009/source/components/Product_WG_List', ["require", "exports", "T009/source/modules/global", "Global/source/modules/api", "handlebars", "Global/libs/zepto.picLazyLoad"], function (require, exports, Global, Api, Handlebars) {
    "use strict";
    ...
    var Product_WG_List = /** @class */ (function () {
        ...
        Product_WG_List.prototype.openProductUrl = function (item) {
            var _this = this;
            Global.Log.log("openProductUrl=%s", item.enname);
                ...
                Api.account.profile_baseInfo(true)
                    ...
                    if (Global.App.accessCheck(Com_Gametree_Cashap.SiteConfig.memberRuleId.WG, baseInfo.memberLevel)) {
                        if (baseInfo.memberLevel == Global.MemberLevel.trial) {
                            ...
                            var url = _this.loginUrl + "?id=" + encodeURI(item.enname) + "&_v=" + (new Date()).valueOf();
                            window.open(url);
                        }
                    }
                });
            ...
            // 		}
            // 		else {
            // 			var url = this.loginUrl + "?id="+ encodeURI(item.enname) +"&_v="+(new Date()).valueOf();
            //
            // 			window.open(url);
            // 		}
            // 	});
...
//# sourceMappingURL=/SGMobile_H5App_V1/T009/source/components/Product_WG_List.js.map