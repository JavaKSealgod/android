import { ConfirmDialog } from '../../../T004/source/modules/global';
/**
 * 线上取款（我要取款）
 */
...
class withdrawalPage {
	...
	drawingsBank_list_callback(result){
		...
		let bankItem_hight = $(this.bankItem).height();

		var data = [],
			amountPerGroup = (banklist_hight - banklist_hight%bankItem_hight)/bankItem_hight || $(this.bankGroupTpl).data("group-amount") || 7,
			arr = [],
			cache = [];
